"""
Code Agent MCP Configuration System
...
from .logging import LogLevel, StructuredLogger, create_logger
...
class MCPConfig(BaseModel):
    ...
    def validate_transport(cls, v: Any) -> MCPTransportType:
        ...
        if isinstance(v, MCPTransportType):
            return v
    ...
    def to_mcp_configs(self) -> list[MCPConfig]:
        ...
        for name, server_config in self.mcpServers.items():
            ...
            if "command" in server_config:
                ...
                # Distinguish between SSE and HTTP based on URL or explicit transport
...
class MCPConfigLoader:
    ...
    def __init__(self, logger: StructuredLogger | None = None):
        ...
        """
        self.logger = logger or create_logger("mcp_config", LogLevel.INFO)
    ...
    def load_from_file(self, config_path: str | Path) -> list[MCPConfig]:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(enabled_configs)} enabled MCP servers (out of {len(configs)} total)",
                servers=[c.name for c in enabled_configs],
            )
    ...
    def validate_config(self, config: MCPConfig) -> tuple[bool, list[str]]:
        """
        ...
        try:
            ...
            elif config.transport in (MCPTransportType.SSE, MCPTransportType.STREAMABLE_HTTP):
                if not config.url:
                    errors.append(f"{config.transport.value} transport requires 'url'")
